...
```{r}
#library(gaggle) ;library(predictionet)
...
library(DLBCL);library(pathview);library(AnnotationDbi)
library(org.Hs.eg.db);library(limma);library(edgeR)
library(multtest);library(nortest);library(outliers)
library(igraph);library(tidyverse);library(pvclust)
library(cummeRbund);library(pastecs);library(psych)
library(genefilter);library(Hmisc);library(ellipse)
library(RColorBrewer);library(VennDiagram)
library(cummeRbund);library(BioNet)
...
go.sets.BP = go.sets.hs[go.subs.hs$BP]
go.sets.CC = go.sets.hs[go.subs.hs$CC]
go.sets.MF = go.sets.hs[go.subs.hs$MF]

go.bp.sym<-lapply(go.sets.BP, eg2sym)
go.mf.sym<-lapply(go.sets.MF, eg2sym)
go.cc.sym<-lapply(go.sets.CC, eg2sym)

# sequence data
genome.fa="/media/drew/umb_triley/umb_triley/ReferenceGenomes/UCSC_hg38/genome.fa"
refgtf="/media/drew/umb_triley/umb_triley/ReferenceGenomes/UCSC_hg38/genes.gtf"
inDir="/media/drew/umb_triley/umb_triley/urine1/cuffdiff_results_hg38_default/LUTS-over-CTRL"
cuff<-cummeRbund::readCufflinks(dir=inDir,genome=genome.fa,gtfFile=refgtf, rebuild=F)

gtfDir="/media/drew/umb_triley/umb_triley/urine1/cuffcompare_results_hg38_gtf_guided"
gtffile <- file.path(gtfDir,"cuffcmp.combined.gtf")
cuffcmp="/media/drew/umb_triley/umb_triley/urine1/cuffcompare_results_hg38_gtf_guided/cuffcmp.combined.gtf"
...
txdb<-makeTxDbFromGFF(file ="/media/drew/umb_triley/umb_triley/urine1/cuffcompare_results_hg38_gtf_guided/cuffcmp.combined.gtf", format="gtf", circ_seqs=character(),organism = "Homo sapiens")
...
#For genome annotation purposes, one may be interested in the gene models, which are provided in the 
...
library("biomaRt")
ensembl <- useMart(biomart = "ensembl",dataset = 'ENSEMBL_MART_SNP' )
 
                   dataset = "hsapiens_gene_ensembl", 
                   mirror = "useast")
ensembl=useMart("ensembl")
ensembl.snp <- useEnsembl(biomart = "ensembl", dataset = 'ENSEMBL_MART_SNP')
snps<-useMart("ENSEMBL_MART_SNP")
grch38.snps<-listDatasets(snps)
grch38.SNPs<-grch38.snps[12,]
#building a query, requires filters, attributes and values
#listAttributes shows all attributes
grch38snps<-grch38.snps$dataset[12]
snps@dataset
grch38snps
hsSNPs<-grch38.snps$dataset[13]
head(hsSNPs)
library("biomaRt")
som.sv.mart = useMart("ENSEMBL_MART_SNP",dataset = "hsapiens_structvar_som")
sv.mart = useMart("ENSEMBL_MART_SNP", dataset="hsapiens_structvar")
ensembl.snp = useMart("ENSEMBL_MART_SNP", dataset = "hsapiens_snp")
som.ensembl.snp = useMart("ENSEMBL_MART_SNP", dataset = "hsapiens_snp_som")
listDatasets(som.ensembl.snp)
listFilters(som.ensembl.snp)
...
my_snp_detail <- getBM(attributes=c("refsnp_id","allele","chrom_start"),
                       ...
                       values=my_snp,
                       mart=snp)
attributes = listAttributes(grch38.snps)
...
```{r import-gene-expr}
genome="/media/drew/umb_triley/umb_triley/Reference-Genomes/Human/UCSC_hg38/genome.fa"
refgtf="/media/drew/umb_triley/umb_triley/Reference-Genomes/Human/UCSC_hg38/genes.gtf"
inDir="/media/drew/umb_triley/umb_triley/urine1/cuffdiff_results_hg38_default/LUTS-over-CTRL/"

cuff<-readCufflinks(dir=inDir,genome="/media/drew/umb_triley/umb_triley/Reference-Genomes/Human/UCSC_hg38/genome.fa",gtfFile="/media/drew/umb_triley/umb_triley/Reference-Genomes/Human/UCSC_hg38/genes.gtf", rebuild=F)
