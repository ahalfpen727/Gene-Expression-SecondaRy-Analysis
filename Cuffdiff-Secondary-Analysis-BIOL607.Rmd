...
```{r data-initialize, fig.margin=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r filtering-data}
library(edgeR)
d <- DGEList(counts = g.cnt.ma, group=groups)
...
table(p.adjust(de$table$PValue<0.01))
d <- calcNormFactors(d)
...
plot(d$AveLogCPM, log2(mock_mean), pch='.')## filter the data enough to perform Shapiro-Wilk normality test
...
dgel
library(tree); library(rpart.plot); library(rpart)
tree1 <- tree(over.group ~ under.group, data = as.data.frame(dgel$counts))
summary(tree1)
plot(tree1)
text(tree1)
...
g.cnt.f_bh <- filtered_(g.cnt.f.pvals, method="BH",
